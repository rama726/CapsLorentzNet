...
enc2 = OneHotEncoder(handle_unknown='ignore').fit([[11],[13],[22],[130],[211],[321],[2112],[2212]])
...
def collate_fn(data, scale=1., nobj=None, edge_features=[], add_beams=False, beam_mass=1):
    ...
    if add_beams:
        ...
        mass1=data["mass"]
        addmass = torch.tensor([[beam_mass, beam_mass]],dtype=data['mass'].dtype).expand(data['mass'].shape[0],2)
        mass=torch.cat([addmass, mass1], dim=-1)
        mass = mass.unsqueeze(-1) # [B,N,1]
        mass_tensor = mass.view(mass.shape + (1,))
        nodes1 = (one_hot.unsqueeze(-1) * mass_tensor).view(mass.shape[:2] + (-1,)) # [B,N,2]
        target1=data["target"].unsqueeze(1)
        target=torch.cat([target1] * nodes1.shape[1], dim=1)
        nodes=torch.cat([nodes1, target], dim=-1)
    else:
        pid = torch.from_numpy(np.abs(np.asarray(data["mass"], dtype=int))).unsqueeze(-1)
        one_hot = enc2.transform(pid.reshape(-1,1)).toarray().reshape(pid.shape[:2]+(-1,))
